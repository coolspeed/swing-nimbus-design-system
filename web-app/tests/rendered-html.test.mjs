...
test("keeps the design-system tokens and interactive contracts explicit", async () => {
  ...
  assert.match(css, /--nimbus-control-highlight:\s*#f2f5f8/i);
  assert.match(css, /--nimbus-control-mid:\s*#dae3eb/i);
  assert.match(css, /--nimbus-control-low:\s*#bcccda/i);
  ...
  assert.match(css, /\.nimbus-select-arrow\s*\{[\s\S]*?background:\s*linear-gradient\(#ecf1f4 0%,\s*var\(--nimbus-control-mid\) 52%,\s*#c1ceda 100%\)/i);
  assert.match(css, /\.nimbus-select-arrow\s*\{[\s\S]*?width:\s*16px/i);
  ...
  assert.match(css, /\.check-row input\[type="checkbox"\]:checked,[\s\S]*?background:\s*linear-gradient\(var\(--nimbus-control-highlight\) 0%,\s*var\(--nimbus-control-mid\) 58%,\s*var\(--nimbus-control-low\) 100%\)/i);
  assert.match(css, /\.check-row input\[type="checkbox"\]:checked::after\s*\{[\s\S]*?left:\s*2px[\s\S]*?top:\s*3px[\s\S]*?width:\s*9px/i);
  assert.match(css, /\.check-row input\[type="radio"\]:checked::after\s*\{[\s\S]*?left:\s*3px[\s\S]*?top:\s*3px[\s\S]*?border-radius:\s*50%[\s\S]*?background:\s*#030304/i);
  ...
  assert.match(css, /\.tab-strip button\s*\{[\s\S]*?height:\s*24px[\s\S]*?border:\s*1px solid #343a40/i);
  assert.match(css, /\.tab-strip button\[aria-selected="true"\]\s*\{[\s\S]*?border-color:\s*#202b35[\s\S]*?linear-gradient\(#e0e8ee 0%,\s*#c9d6e0 36%,\s*#b7cadd 66%,\s*#a2b9cc 100%\)/i);
