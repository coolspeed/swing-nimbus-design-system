...
test("keeps the design-system tokens and interactive contracts explicit", async () => {
  ...
  assert.match(css, /animation:\s*indeterminate \.4s linear infinite/i);
  assert.match(css, /\.progress-track\.indeterminate\s*\{[\s\S]*?height:\s*13px[\s\S]*?overflow:\s*hidden[\s\S]*?#fff 10%,[\s\S]*?#dee0e3 52%,[\s\S]*?#fbfcff 100%[\s\S]*?inset 0 2px rgba\(255,255,255,\.82\)[\s\S]*?inset 0 3px rgba\(84,89,96,\.12\)[\s\S]*?inset 0 -1px rgba\(95,101,109,\.34\)/i);
  ...
  assert.match(css, /\.progress-indeterminate-pattern::before\s*\{[\s\S]*?inset:\s*6\.5px 0 auto[\s\S]*?height:\s*1px[\s\S]*?linear-gradient\(#e0a05e,\s*#c36903 54%,\s*#df8120\)/i);
  ...
  assert.match(css, /clip-path:\s*path\("M -1 6 C 1\.5 6 2\.5 6 4 5\.5/i);
  assert.match(css, /C 12 1\.5 13\.5 1\.5 15 1\.5/);
  assert.match(css, /C 27\.5 6 28\.5 6 30 6 L 30 8/i);
  ...
  assert.match(css, /\.fake-scrollbar \.scroll-thumb\s*\{[\s\S]*?top:\s*\.5px[\s\S]*?height:\s*16px[\s\S]*?border-radius:\s*5px 5px 22px 22px \/ 3px 3px 14px 14px[\s\S]*?#e6edef 0%,[\s\S]*?#95aec3 32%,[\s\S]*?#dcebf0 100%[\s\S]*?inset 0 0 0 1px rgba\(33,62,84,\.18\)[\s\S]*?inset -1px 0 rgba\(18,40,57,\.3\)[\s\S]*?inset 0 -1px #526f85[\s\S]*?0 1px 1px rgba\(21,39,55,\.22\)/i);
