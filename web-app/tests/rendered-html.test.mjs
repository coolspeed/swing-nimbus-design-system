...
test("keeps the design-system tokens and interactive contracts explicit", async () => {
  const [page, layout, css, packageJson, config, sitesPlugin] = await Promise.all([
  ...
  assert.match(sitesPlugin, /config\.compatibility_flags\?\.length === 0[\s\S]*?delete config\.compatibility_flags/i);
