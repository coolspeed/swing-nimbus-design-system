...
- indeterminate progress는 짧은 사선 stripe나 반원 outline의 연속이 아니다. 캡처처럼
  ...
  44 px 패턴 한 주기는 약 0.7초에 흘러가도록 한다. 주황 stream을 반복 조각으로 구현할 때는
...
- horizontal scrollbar의 화살표 버튼은 트랙 쪽 경계만 오목하게 파고, thumb는 상단이
  ...
  기본 상태는 비교 캡처에 맞춘 절제된 회청색(`#F5F9FA → #A6BED1 → #E9F7FA`)으로 칠한다. 가장 짙은 지점은 높이의 약 32%에 두며, 넓게 퍼지는 바깥 그림자는 1 px·22% 불투명도로 제한하고 표면 안쪽 1 px의 어두운 경계와 `#70899C` 하단 inset으로 입체감을 만든다. 눌린 상태는 `screenshots/scroll bar.png`의 중앙 세로 픽셀을 기준으로 상단 반사색 `#8DA8BE`에서 35% 지점의 `#386791`까지 급격히 어두워졌다가 하단의 `#7AA7D2`로 밝아지는 청회색 프로파일을 사용하며, `#17364D` 외곽선과 `#416484` 하단 inset으로 깊이를 고정한다. 기본 상태는 `#34495C` 외곽선으로 형태를 분명히 한다. 두 상태 모두 상단 모서리는 수평 3 px·수직 2 px만 둥글려 끝의 날카로움만 제거하고, 하단 모서리는 수평 22 px·수직 14 px의 타원 곡률을 사용해 양옆 사선이 길게 들어오도록 한다. rail은 상단 `#68696D`의 깊은 홈에서 하단 canvas `#D6D9DF`로 풀리는 비대칭 그라데이션을 사용한다.
