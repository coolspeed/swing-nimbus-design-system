...
- indeterminate progress는 짧은 사선 stripe나 반원 outline의 연속이 아니다. 캡처처럼
  ...
  탕후루형 실루엣처럼 몸체가 짧게 평평해진 뒤 긴 경사로 목에 이어지게 한다. 최종 경로의 plateau는
  직전 85% 축소 래스터보다 약 15% 긴 `x=13.05–16.95` 범위를 사용해 몸체가 지나치게 뾰족해지지 않게 한다.
...
- indeterminate stream의 14 px 패턴 레이어는 15 px track의 수학적 중앙보다 0.5 px 위에 둔다
  (`top: 50%; margin-top: -7.5px`). 물방울 경로는 원래 형상 대비 수평 약 90%·수직 약 86% 크기의 좌표를
  직접 사용하고 CSS `scale`은 적용하지 않는다. 반복 pitch는 28 px로 고정해 몸체 사이로 가는 core가 드러나게 하며,
  넓어진 track이 위·아래 여유를 제공한다. 곡선 안티앨리어싱과 하단 그림자가 합성된
  최종 래스터에서 주황색 첫 행 위와 마지막 행 아래에 각각 최소 1 px 여백이 남아야 하며, DOM box의
  중심값만으로 대칭을 판정하지 않는다.
- indeterminate stream은 `ORANGE` 토큰(`#BF6204`)을 54% 지점의 최암부로 직접 사용하고, 원본의 밝은 반사를 반영한 전용 주황 프로파일(`#F3CFA0 → #BF6204 → #FBAA4A`)을 사용한다. 레퍼런스의 중앙 최암부 실측값 `#C36903`과 토큰의 차이는 작지만 의미색 일관성을 위해 토큰을 기준점으로 삼는다. 원본 주황 픽셀 평균 `RGB(213,138,53)`에 비해 웹 래스터가 `RGB(206,140,64)`로 덜 붉게 측정될 때는 `rgba(255,120,0,.12)`의 균일 보정층과 `saturate(1.12)`를 함께 적용한다. 배포 래스터의 목표 평균은 약 `RGB(213,139,54)`이며, 이 보정은 밝기 곡선보다 녹색·청색 비율을 낮추는 데 사용한다. 가운데 core는 패턴 정중앙의 `6.8–7.2 px` 범위로 제한하고 물방울 경로의 양끝 목도 정확히 같은 범위에서 시작·종료시킨다. 별도 scale 합성을 하지 않아 core와 목이 서로 다른 서브픽셀 격자에 놓이거나 접합부에 계단이 생기지 않아야 한다. 위에는 밝은 반사선, 아래에는 짙은 drop shadow를 더해 흰 track에서 떠 있는 볼록한 흐름으로 보이게 한다. track 상반부는 `#777C84` 외곽선 다음에 절제된 흰 반사선을 겹치고, 52% 지점의 `#DEE0E3`까지 완만하게 내려가는 얕은 오목면으로 만든다. 중간 inset 암부는 12%, 하단 inset은 34% 수준으로 제한해 stream보다 rail의 입체감이 앞서지 않게 한다.
- horizontal scrollbar의 화살표 버튼은 트랙 쪽 경계만 오목하게 파고, thumb는 상단이
  ...
  화살표와 레일이 만나는 양쪽 끝에는 폭 14 px·높이 13 px의 국소 radial-gradient 캡을 두고,
  `#0E1012`에 가까운 상단 암부가 레일 안쪽과 아래쪽으로 타원형으로 흐려지게 한다. 이 캡은 thumb 아래,
  rail 위에 놓여 원본처럼 화살표의 오목한 절단부와 thumb의 뾰족한 진입부를 시각적으로 완충해야 한다.
