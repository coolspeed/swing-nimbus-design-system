...
- `JComboBox`의 화살표 버튼은 입력 표면과 분리된 우측 17 px 사각 영역으로 둔다. 이 영역은 원본처럼 가운데가 짙은 공통 청회색 크롬 그라데이션(`#D6E2EA → #9FB5C7 → #CEDCE7`)과 `#687C8E` 좌측 경계선, 중앙의 9 px 짙은 아래 화살표를 사용한다. 콤보박스 본체에서 빛이 덜 닿는 63~80% 하부 면을 더 짙게 만들고, 그 아래 반사띠는 좁고 은은하게 복귀시킨다. 이 음영은 표면 내부의 명암이며 오브젝트가 배경에 드리우는 외부 그림자의 크기나 농도를 늘려 표현하지 않는다. 바깥 컨테이너에서 모서리를 잘라 외곽 테두리가 네 귀퉁이에서 끊기지 않게 하며, 브라우저 기본 화살표는 노출하지 않는다.
...
- 비선택 탭도 가느다란 검은 실선처럼 보이지 않도록 약 1.25 px의 회청색 외곽선(`#566572`)을
  사용한다. 선택 탭보다 밝고 조금 가늘어야 하며, 선택 상태의 1.5 px `#22313F` 외곽선과
  명확한 위계를 유지한다.
- 세로 inset은 촘촘하게 유지하고 웹에서 30~31 px를 넘겨 과도한 상하 여백을 만들지 않는다.
- 탭 아래 이중선은 창 가장자리까지 닿게 늘이지 않고 콘텐츠 fieldset처럼 좌우 여백을 남긴다.
  데스크톱의 최대 여백은 12 px이고 좁은 화면에서는 `clamp(4px, 1.2%, 12px)`로 유동적으로
  줄인다. 선은 viewport 폭에 고정하지 않고 `width: max-content; min-width: 100%`인 tab list
  안의 독립된 divider 요소로 둔다. 이 divider가 탭들과 같은 가로 스크롤 좌표계를 사용해야
  모바일에서 마지막 탭까지 선이 계속 이어지며, 어느 스크롤 위치에서도 선 양 끝의 여백이
  전체 탭 rail을 기준으로 보존된다.
...
- indeterminate progress는 짧은 사선 stripe나 반원 outline의 연속이 아니다. 캡처처럼
  ...
  물방울 세로 비율은 86%로 복원해 약 12 px의 볼륨을 유지하고, track 자체를 13 px에서 15 px로 늘려
  위·아래 여유를 확보한다. stream과 그림자가 border 밖으로 나오지 않게
  track에서 clipping한다.
  오렌지색을 track 전체에 채운 뒤 흰 모양을 올리는 역상 구현은 금지한다.
  원형의 44 px 물방울 주기는 몸체를 보존하면서 양끝 목을 줄여 실제 반복 간격 29 px로 압축한다. 압축 뒤에도
  레퍼런스보다 느려 보이지 않도록 한 주기는 약 0.333초에 흘러가게 한다. 이는 0.40초 구현보다 이동 속도를
  정확히 약 20% 높인 값이다. 주황 stream을 반복 조각으로 구현할 때는
  각 조각의 클립 경계를 최소 1 px 겹치고 경계선을 요소 바깥으로 밀며, 원본의 가는 목처럼
  0.5 px 두께의 주황 core를 패턴 전체에 연속으로 이어 둔다. 물방울 몸체의 폭과 곡률은 유지하되
  양끝의 평평한 목만 줄여 조각 폭 30 px·실제 반복 간격 28 px로 배치한다. 인접 조각을 2 px 겹쳐
  방울 사이 목의 시각적 거리를 줄이되, 애니메이션의 소수점 이동 중에도
  주기 경계에 흰색 세로 틈이나 톱니 모양 안티앨리어싱이 드러나지 않아야 한다.
  목에서 몸체로 전환되는 두 Bézier의 접선 기울기는 완전히 연속시키지 않고 완만한 각도 차를 둬,
  rounding은 유지하면서도 사인파처럼 지나치게 미끈해지지 않는 작은 어깨를 만든다. 정상부의 평평한
  구간은 조각 중심의 약 2 px로 제한하고, 양쪽 상승·하강 구간을 각각 약 8 px까지 늘려 레퍼런스의
  탕후루형 실루엣처럼 몸체가 짧게 평평해진 뒤 긴 경사로 목에 이어지게 한다.
  각 반복 주황 물방울 내부에는 본체보다 한 단계만 밝고 붉은 기가 살짝 도는 오렌지(`rgba(238,159,78,.78)`)의
  tone-on-tone 세로 highlight를 1 px 두 줄씩 두고 stream과 같은 속도·방향으로 이동시킨다.
  크림색에 가까운 고명도 반사선을 사용해 물방울과 과도한 색상 차를 만들지 않는다.
- indeterminate stream의 14 px 패턴 레이어는 15 px track의 수학적 중앙보다 0.5 px 위에 둔다
  (`top: 50%; margin-top: -7.5px`). 물방울은 중앙 기준 `scaleX(.85) scaleY(.86)`으로 수평 폭을 15% 줄이고
  약 12 px 가시 높이를 유지한다. 반복 pitch는 28 px로 고정해 축소된 몸체 사이로 가는 core가 더 드러나게 하며,
  넓어진 track이 위·아래 여유를 제공한다. 곡선 안티앨리어싱과 하단 그림자가 합성된
  최종 래스터에서 주황색 첫 행 위와 마지막 행 아래에 각각 최소 1 px 여백이 남아야 하며, DOM box의
  중심값만으로 대칭을 판정하지 않는다.
- indeterminate stream은 `ORANGE` 토큰(`#BF6204`)을 54% 지점의 최암부로 직접 사용하고, 원본의 밝은 반사를 반영한 전용 주황 프로파일(`#F3CFA0 → #BF6204 → #FBAA4A`)을 사용한다. 레퍼런스의 중앙 최암부 실측값 `#C36903`과 토큰의 차이는 작지만 의미색 일관성을 위해 토큰을 기준점으로 삼는다. 원본 주황 픽셀 평균 `RGB(213,138,53)`에 비해 웹 래스터가 `RGB(206,140,64)`로 덜 붉게 측정될 때는 `rgba(255,120,0,.12)`의 균일 보정층과 `saturate(1.12)`를 함께 적용한다. 배포 래스터의 목표 평균은 약 `RGB(213,139,54)`이며, 이 보정은 밝기 곡선보다 녹색·청색 비율을 낮추는 데 사용한다. 가운데 core는 패턴 정중앙을 유지하되 0.4 px로 제한해 몸체보다 앞서 보이지 않게 한다. 위에는 밝은 반사선, 아래에는 짙은 drop shadow를 더해 흰 track에서 떠 있는 볼록한 흐름으로 보이게 한다. track 상반부는 `#777C84` 외곽선 다음에 절제된 흰 반사선을 겹치고, 52% 지점의 `#DEE0E3`까지 완만하게 내려가는 얕은 오목면으로 만든다. 중간 inset 암부는 12%, 하단 inset은 34% 수준으로 제한해 stream보다 rail의 입체감이 앞서지 않게 한다.
...
- 파일 선택기 도구 아이콘은 문자 글리프나 플랫폼 이모지로 대체하지 않는다. `04.png`의
  청회색 폴더와 파란 위쪽 화살표, 황금색 집, 오렌지 별표가 붙은 청회색 폴더, 회청색 목록 문서,
  격자 문서와 확대경 실루엣을 고정 크기 도형으로 재현한다. 외곽선은 짙은 회청색, 내부는 작은
  명암 그라데이션을 사용해 Nimbus의 16~21 px 래스터 아이콘 톤을 유지하며 글꼴 설치 상태에 따라
  모양이 바뀌어서는 안 된다.
- 내부 프레임은 복수 문서를 동시에 다루는 MDI 작업 공간에서만 사용한다. 제목 표시줄을
  포인터로 드래그하면 desktop 경계 안에서 이동하고, 클릭·포커스로 활성 창과 z-order가 바뀌어야 한다.
  제목 표시줄은 드래그 중에도 Nimbus 원본처럼 일반 화살표 커서(`default`)를 유지하며 이동용 십자
  커서(`move`)로 바꾸지 않는다.
