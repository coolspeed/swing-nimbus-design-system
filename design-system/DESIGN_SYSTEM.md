...
- horizontal scrollbar의 화살표 버튼은 트랙 쪽 경계만 오목하게 파고, thumb는 상단이
  ...
  기본 상태는 비교 캡처에 맞춘 절제된 회청색(`#F5F9FA → #A6BED1 → #E9F7FA`)으로 칠한다. 가장 짙은 지점은 높이의 약 32%에 두되 바깥 그림자와 좌우 inset 음영은 낮은 불투명도로 유지하고, 눌린 상태에서만 더 짙은 대비를 사용한다. `#34495C` 외곽선으로 형태를 분명히 한다. 하단 모서리는 수평 22 px·수직 14 px의 타원 곡률을 사용해 양옆 사선이 길게 들어오도록 하고, rail은 상단 `#68696D`의 깊은 홈에서 하단 canvas `#D6D9DF`로 풀리는 비대칭 그라데이션을 사용한다.
