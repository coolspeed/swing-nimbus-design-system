...
- horizontal scrollbar의 화살표 버튼은 트랙 쪽 경계만 오목하게 파고, thumb는 상단이
  ...
  따라 세로 크기를 늘리지 않는다. 레일은 15 px 컨트롤 박스 상단에서 1 px 내려 시작하고 하단으로
  1 px 더 이어지는 독립 레이어로 그린다. 이로써 화살표와 thumb의 크기·기준선을 움직이지 않은 채
  위쪽 돌출 없이 레일의 하단만 1 px 보강한다. thumb는 레일과 같은 `top: 1px`에서 시작하며
  15 px 높이를 유지한다. 확장된 레일 하단에 정확히 맞으므로 두께를 줄이지 않고도 레일 위로
  솟거나 아래로 돌출되지 않는다. 오목한 경계는 트랙색 도형을 버튼 위에 덧칠해 흉내
