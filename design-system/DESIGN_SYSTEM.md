...
- indeterminate stream의 14 px 패턴 레이어는 15 px track의 수학적 중앙보다 0.5 px 위에 둔다
  ...
  중심값만으로 대칭을 판정하지 않는다.
- indeterminate stream은 determinate보다 밝은 전용 주황 프로파일(`#F0C995 → #C36903 → #F8AA48`)을 사용하고, 위에는 밝은 반사선, 아래에는 짙은 drop shadow를 더해 흰 track에서 떠 있는 볼록한 흐름으로 보이게 한다. track 상반부는 `#777C84` 외곽선 다음에 절제된 흰 반사선을 겹치고, 52% 지점의 `#DEE0E3`까지 완만하게 내려가는 얕은 오목면으로 만든다. 중간 inset 암부는 12%, 하단 inset은 34% 수준으로 제한해 stream보다 rail의 입체감이 앞서지 않게 한다.
