...
- indeterminate progress는 짧은 사선 stripe나 반원 outline의 연속이 아니다. 캡처처럼
  ...
  세로는 72%로 축소해 물방울의 위·아래를 각각 약 1 px 더 비우고 track 내부에 맞춘다. stream과 그림자가 border 밖으로 나오지 않게
  track에서 clipping한다.
...
- indeterminate stream의 14 px 패턴 레이어는 13 px track의 수학적 중앙보다 0.5 px 위에 둔다
  (`top: 50%; margin-top: -7.5px`). 물방울은 중앙 기준 `scaleY(.72)`로 줄여 원래 12 px가량이던
  가시 높이를 약 10 px로 만들고 위·아래 여유를 각각 약 1 px 늘린다. 곡선 안티앨리어싱과 하단 그림자가 합성된
