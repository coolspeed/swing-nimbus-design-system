...
- 선택 탭은 캡처에서 측정한 낮은 채도의 청회색(`#F3F5F8 → #DCE4EB → #CCD8E2 →
  #BDCDDA`)처럼 위에서 아래로 단조롭게 어두워져야 한다. 중간 이후 다시 밝아지는 glossy
  반사띠를 만들지 않고, 짙은 `#355D7C` 외곽선으로 선택 상태를 구분한다.
...
- indeterminate progress는 짧은 사선 stripe나 반원 outline의 연속이 아니다. 캡처처럼
  ...
  44 px 패턴 한 주기는 약 0.8초에 흘러가도록 한다. 각 반복
