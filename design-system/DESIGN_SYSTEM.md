...
- indeterminate progress는 짧은 사선 stripe나 반원 outline의 연속이 아니다. 캡처처럼
  ...
  각 반복 주황 물방울 내부에는 본체보다 한 단계만 밝은 오렌지(`rgba(242,173,88,.78)`)의
...
- indeterminate stream의 14 px 패턴 레이어는 15 px track의 수학적 중앙보다 0.5 px 위에 둔다
  ...
  중심값만으로 대칭을 판정하지 않는다.
- indeterminate stream은 `ORANGE` 토큰(`#BF6204`)을 54% 지점의 최암부로 직접 사용하고, 원본의 밝은 반사를 반영한 전용 주황 프로파일(`#F3CFA0 → #BF6204 → #FBAA4A`)을 사용한다. 레퍼런스의 중앙 최암부 실측값 `#C36903`과 토큰의 차이는 작지만, 의미색 일관성을 위해 토큰을 기준점으로 삼고 주변 반사색의 녹색·청색 성분을 높여 전체가 지나치게 갈색으로 보이지 않게 한다. 위에는 밝은 반사선, 아래에는 짙은 drop shadow를 더해 흰 track에서 떠 있는 볼록한 흐름으로 보이게 한다. track 상반부는 `#777C84` 외곽선 다음에 절제된 흰 반사선을 겹치고, 52% 지점의 `#DEE0E3`까지 완만하게 내려가는 얕은 오목면으로 만든다. 중간 inset 암부는 12%, 하단 inset은 34% 수준으로 제한해 stream보다 rail의 입체감이 앞서지 않게 한다.
